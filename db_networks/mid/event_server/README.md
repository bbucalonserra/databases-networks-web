...
Once the server is running, you can test the following main routes (it's possible to move directly from the webpages with hyperlinks):
...
* S**erver-Side Search Engine**: Located at /attendee. It uses a server-side SQL LIKE query to filter events by title or description based on user input (req.query.q).
* **Advanced SQL** (My Bookings History): Located at /attendee/my-bookings. This route performs a SQL JOIN to retrieve bookings linked to event details, demonstrating relational database management.
* **Data Integrity Logic** (Sold Out): The server dynamically checks ticket availability. If an event reaches 0 tickets, the booking button is disabled via server-side logic to maintain data integrity.
